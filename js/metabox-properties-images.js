jQuery(document).ready(function ($) {
		...
		galleryFrame.on('select', function () {
			...
			selection.map(function (attachment) {
				...
				if (
					...
					// force everything in the selection to be an integer
					selectedImageIds = selectedImageIds.map(function (x) {
	...
	galleryContainer.on('click', '.remove-image', function () {
		...
		var imageId = parseInt(imageDiv.data('id'), 10);

		selectedImageIds = galleryIdsField.val().split(',').map(Number);
	...
	// Make the images drag-droppable to change the order
	galleryContainer.sortable({
		update: function (event, ui) {
			// Get the updated order of the images
			var newOrder = galleryContainer.sortable('toArray', {
				attribute: 'data-id',
			});
			// Update the hidden input field with the new order
			galleryIdsField.val(newOrder.join(','));
		},
	});
});
