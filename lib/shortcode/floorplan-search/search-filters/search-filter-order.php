...
function rentfetch_search_floorplans_filters() {
    ...
    var_dump( $options_floorplan_filters );
                
    if ( !empty( $options_floorplan_filters ) && in_array( 'beds_search', $options_floorplan_filters ) )
        add_action( 'rentfetch_do_search_floorplans_filters', 'rentfetch_search_floorplans_filters_beds' );
    
    if ( !empty( $options_floorplan_filters ) && in_array( 'baths_search', $options_floorplan_filters ) )
        add_action( 'rentfetch_do_search_floorplans_filters', 'rentfetch_search_floorplans_filters_baths' );
    
    if ( !empty( $options_floorplan_filters ) && in_array( 'price_search', $options_floorplan_filters ) )
        add_action( 'rentfetch_do_search_floorplans_filters', 'rentfetch_search_floorplans_filters_price' );
    
    if ( !empty( $options_floorplan_filters ) && in_array( 'date_search', $options_floorplan_filters ) )
        add_action( 'rentfetch_do_search_floorplans_filters', 'rentfetch_search_floorplans_filters_date' );
    
    if ( !empty( $options_floorplan_filters ) && in_array( 'squarefoot_search', $options_floorplan_filters ) )
        add_action( 'rentfetch_do_search_floorplans_filters', 'rentfetch_search_floorplans_filters_squarefoot' );
