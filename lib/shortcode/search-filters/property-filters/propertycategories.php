 ...
 * @return void
 */
function rentfetch_search_filters_property_categories() {
...
function rentfetch_search_properties_args_categories( $property_args ) {
	...
	if ( ! wp_verify_nonce( $nonce, 'rentfetch_frontend_nonce_action' ) ) {
		die( 'Nonce verification failed (propertycategories)' );
	}
...
add_filter( 'rentfetch_search_property_map_properties_query_args', 'rentfetch_search_properties_args_categories', 10, 1 );
