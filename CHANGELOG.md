...
- Hardened settings saves with explicit request-method, administrator capability, and nonce verification.
- Added strict validation for Rent Fetch search parameters while preserving existing custom extension parameters unchanged.
- Bounded the separate search-analytics storage copy to prevent unrestrained transient growth.
...
- Republished the plugin from a clean foundation with no functional changes.
...
- Added unique targetable classes for single-property sidebar address, website, phone, and email content and links.
- Improved default floor plan grid card spacing so the divider sits evenly between floor plan attributes and pricing.
...
- Refined single-property template spacing, section heading dividers, and property detail statistics layout.
- Added Rent Fetch spacing tokens and responsive font-size tokens for more consistent frontend styling overrides.
- Updated default floorplan card styling to use borders by default with hover-only shadow treatment.
- Reduced property and floorplan filter font sizing and softened property fee table dividers.

## 0.35.2

- Scoped floorplan search filter options to the current property or property list when a search is property-specific.
- Kept global floorplan filter option caching for unscoped searches while bypassing persistent cache for property-scoped options.
- Hid beds and baths filters when fewer than two usable choices are available.

## 0.35.1

- Improved property and floorplan search failure handling with more specific frontend messages and server-side error references in the PHP log.
- Adjusted floorplan card overlay stacking and attribute separator spacing.
...
- Added support for Yardi lease fees so synced fee data can be used in property pricing and fee displays.
- Updated fee-source priority so live synced lease fees are used when available, while empty or incomplete sync data will no longer override manual fee settings.
- Improved the property editor fee preview so it more clearly reflects the fee source and pricing details being used on the frontend.

## 0.34.1

- Improved settings page routing so missing, legacy, or unknown tab and section parameters resolve to a valid settings section.
- Fixed saving for Google Maps settings when the Property Settings page is loaded without an explicit section parameter.

## 0.34.0

- Added a signed monitoring status endpoint so the Rent Fetch API can pull current site health without relying on the existing sync bootstrap route.
- Added site-wide monitoring rollups, including property-level health summaries, endpoint failure aggregation, and exception payloads for dashboarding.
- Improved monitoring support for child record issues by rolling floorplan and unit endpoint failures up to the related property.

## 0.33.4

- Added aggregate sync-status support so admin sync colors use normalized sync metadata instead of parsing raw API payloads.
- Updated sync handling compatibility for properties, floorplans, and units across the Rent Fetch Sync integrations.

## 0.33.3

- Fixed a typo in shortcode documentation text ("shows" instead of "shwos").
- Updated "Floor Plans" capitalization in the single-property floorplans section heading and subnav label.

## 0.33.2

- Improved Google Maps enqueues so map assets load more reliably in the property search and single-property map contexts.
...
- Minor style updates and fixes to the properties and flooplans shortcodes (the "copied" indicator wasn't displaying properly).
- Adding variables for use in SEOPress.

## 0.32.4

- Adding styles to harden our fees tables (we're seeing some theme defaults override things they shouldn't).
- Removing duplicate fees notes (accidentally added this twice in contexts where the site is a single-property site).
